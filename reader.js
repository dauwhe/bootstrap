...
// create nav wrapper
  ...
  newNav.setAttribute('role', 'doc-toc');
  
  document.getElementsByTagName('body')[0].appendChild(newNav);
...
manifest.then(json => {
  ...
  link.setAttribute('href', json[i].href);
  link.innerHTML = json[i].title;
  newList.appendChild(listItem);
  var iframe= document.createElement('iframe');
  iframe.setAttribute('name', json[i].href);
   iframe.setAttribute('id', json[i].href);
    iframe.setAttribute('src', json[i].href);
    mainFrame.appendChild(iframe);
  };
window.location.hash = json[0].href;
});
...
var current = json.map((el) => el.href).indexOf(currentTarget);


window.location.hash = json[current + 1].href;
